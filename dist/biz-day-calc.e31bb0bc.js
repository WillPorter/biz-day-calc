...
},{}],"node_modules/moment-holiday/build/moment-holiday-us.min.js":[function(require,module,exports) {
var __dirname = "/Users/will.porter/web/zz_practice/biz-day-calc/node_modules/moment-holiday/build";
//! moment-holiday.js
//! version : 1.5.1
...
var btn = document.querySelector('#biz-button');

var endDateFinder = function endDateFinder(date, days) {
  ...
  date = (0, _moment.default)(date).format();
  return date;
};

var findHolidays = function findHolidays(start, end) {
  var holidays = momenth(start).holidaysBetween(end);
  return holidays;
};

var addHolidays = function addHolidays(date, days) {
  date = (0, _moment.default)(date); // use a clone
...
btn.addEventListener('click', function (event) {
  var msg = '';
  var startDateRaw = document.querySelector('.start-date').value;
  var startDate = (0, _moment.default)(startDateRaw);
  var hours = document.querySelector('.hours').value;
  var daysInput = Math.ceil(hours / 6); //target errors or results if they exist from previous attempts

  var errorRemoval = document.querySelector('.error-message');
  var resultRemoval = document.querySelector('.results'); //remove previous messages if they exist
  ...
  if (startDateRaw == '') {
    ...
    errorNotice.innerHTML = msg; //add the errornotice div to the end of the contents of the body tag
  ...
  if (isNaN(hours) || hours == '') {
    var _errorNotice = document.createElement('div');

    _errorNotice.classList.add('error-message');

    msg = 'please enter a valid number for the "Estimated project hours" field';
    _errorNotice.innerHTML = msg; //add the errornotice div to the end of the contents of the body tag

    document.body.appendChild(_errorNotice);
    return;
  } //run the function to the the ending date of project and store it in raw end date


  var rawEndDate = endDateFinder(startDate, daysInput);
  var rawEndDateFormatted = (0, _moment.default)(rawEndDate).format('dddd, MMMM Do YYYY');
  console.log(rawEndDateFormatted); //find the holidays in between the raw end date

  var holidays = findHolidays(startDate, rawEndDate);
  console.log(holidays);
  var numberOfHolidays = holidays.length;
  console.log(numberOfHolidays); //factor in number of holidays into project end date

  var endDateIncHolidays = addHolidays(rawEndDate, numberOfHolidays);
  var endDateFormatted = (0, _moment.default)(endDateIncHolidays).format('dddd, MMMM Do YYYY');
  console.log(endDateFormatted); //create div to display ending date

  var resultDiv = document.createElement('div');
  resultDiv.classList.add('results');
  resultDiv.innerHTML = "Your project will be finished on: ".concat(endDateFormatted, " <br/>\n            There were ").concat(numberOfHolidays, " vacation days factored into this date.<br/>\n            Not including vacation days the project would have ended on: ").concat(rawEndDateFormatted, "\n            ");
  document.body.appendChild(resultDiv);
});
},{"moment":"node_modules/moment/moment.js","moment-holiday":"node_modules/moment-holiday/build/moment-holiday-us.min.js"}],"../../../.config/yarn/global/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
...
//# sourceMappingURL=/biz-day-calc.e31bb0bc.js.map